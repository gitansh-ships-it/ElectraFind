...
function vars(s){let w=s.toLowerCase(),a=[w];if(w.includes("ee"))a.push(w.replace(/ee/g,"i"));if(w.includes("i"))a.push(w.replace(/i/g,"ee"));if(w.includes("sh"))a.push(w.replace(/sh/g,"s"));if(w.includes("s"))a.push(w.replace(/s/g,"sh"));if(w.includes("v"))a.push(w.replace(/v/g,"w"));if(w.includes("w"))a.push(w.replace(/w/g,"v"));if(w.includes("agrawal"))a.push(w.replace(/agrawal/g,"agarwal"));if(w.includes("agarwal"))a.push(w.replace(/agarwal/g,"agrawal"));if(w.includes("agraval"))a.push(w.replace(/agraval/g,"agarwal"));return a}
...
function fuzzyRoman(q, candidate){let a=romanKey(q),b=romanKey(candidate);if(!a||!b)return false;if(a===b||a.includes(b)||b.includes(a))return true;const at=a.split(" "),bt=b.split(" ");if(at.length!==bt.length)return false;let total=0;for(let i=0;i<at.length;i++){const x=at[i],y=bt[i],d=lev(x,y);if(d>Math.max(2,Math.floor(Math.min(x.length,y.length)*.34)))return false;total+=d;}return total<=Math.max(3,Math.floor(a.replace(/ /g,"").length*.22));}
function fuzzyHindi(q,candidate){q=norm(q);candidate=norm(candidate);if(!q||!candidate)return false;if(q===candidate||candidate.includes(q)||q.includes(candidate))return true;const a=q.split(" "),b=candidate.split(" ");if(a.length!==b.length)return false;let total=0;for(let i=0;i<a.length;i++){const x=a[i],y=b[i],d=lev(x,y);const lim=x.length<=3?1:Math.max(1,Math.floor(Math.min(x.length,y.length)*.28));if(d>lim)return false;total+=d;}return total<=Math.max(1,Math.floor(q.replace(/ /g,"").length*.18));}
function match(v,q){if(selectedPart&&String(get(v,"part_no","part"))!==String(selectedPart))return false;let n=norm(q);if(mode==="epic")return ep(get(v,"epic")).includes(ep(q));if(mode==="serial"){let qn=n.replace(/[^0-9]/g,"");return String(get(v,"serial_no","serial"))===qn||String(get(v,"serial_no","serial"))===n;}if(mode==="part"){let qp=n.replace(/[^0-9]/g,"");return String(get(v,"part_no","part"))===qp||String(get(v,"part_no","part"))===n;}if(mode==="house"){let h=norm(get(v,"house_no","house"));return h.includes(n)||h.replace(/\s+/g,"").includes(n.replace(/\s+/g,""));}if(mode==="relation"){let rh=norm(get(v,"relation_name_hindi","relation")),re=norm(get(v,"relation_name_english"));return rh.includes(n)||re.includes(n)||vars(n).some(x=>re.includes(x));}let nh=norm(get(v,"name_hindi","name")),ne=norm(get(v,"name_english","english_name")),sv=norm(get(v,"search_variants","search_text")),rh=norm(get(v,"relation_name_hindi"));if(nh.includes(n)||ne.includes(n)||sv.includes(n)||rh.includes(n))return true;
if(fuzzyHindi(n,nh))return true;let qRoman=/^[a-z0-9 ]+$/i.test(n);if(qRoman){let qk=romanKey(n),ek=romanKey(ne),sk=romanKey(sv);if(qk&&((ek&&(ek.includes(qk)||qk.includes(ek)))||(sk&&(sk.includes(qk)||qk.includes(sk)))))return true;if(fuzzyRoman(n,ne)||fuzzyRoman(n,sv))return true;}return vars(n).some(x=>ne.includes(x)||sv.includes(x)||fuzzyRoman(x,ne)||fuzzyRoman(x,sv));}
function esc(x){return String(x??"").replace(/[&<>"']/g,m=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#039;"}[m]))}
function score(v,q,n){let sc=100,epVal=ep(get(v,"epic")),qEp=ep(q);if(qEp&&epVal===qEp)return 1;if(mode==="serial"&&String(get(v,"serial_no"))===n)return 2;let nh=norm(get(v,"name_hindi")),ne=norm(get(v,"name_english"));if(nh===n||ne===n)return 3;if(nh.startsWith(n)||ne.startsWith(n))return 4;if(nh.includes(n)||ne.includes(n))return 5;if(/^[a-z0-9 ]+$/i.test(n)){let qk=romanKey(n),ek=romanKey(ne);if(qk&&ek&&(ek.includes(qk)||qk.includes(ek)))return 6;}return sc;}
function search(){let q=$("#q").value.trim();if(!q&&!selectedPart){$("#results").innerHTML='<div class="empty">ऊपर से खोज शुरू करें</div>';$("#count").textContent="";return}let n=norm(q);let r=V.filter(v=>(q?match(v,q):(!selectedPart||String(get(v,"part_no","part"))===String(selectedPart)))).sort((a,b)=>score(a,q,n)-score(b,q,n)).slice(0,250);$("#count").textContent=r.length?`(${r.length}${r.length===250?"+":""})`:"";$("#results").innerHTML=r.length?r.map(v=>`<article class="card glass" data-id="${esc(v.id)}"><div class="name">${esc(get(v,"name_hindi","name")||"नाम उपलब्ध नहीं")}</div><div style="font-size:13px;color:var(--t2);margin:3px 0 6px;">पिता/पति का नाम: <b>${esc(get(v,"relation_name_hindi")||"—")}</b></div><div class="pills"><span class="pill">भाग: <b>${esc(get(v,"part_no")||"—")}</b></span><span class="pill">क्रमांक: <b>${esc(get(v,"serial_no")||"—")}</b></span><span class="pill">मकान: <b>${esc(get(v,"house_no")||"—")}</b></span><span class="pill">EPIC: <b>${esc(get(v,"epic")||"—")}</b></span><span class="pill">उम्र: ${esc(get(v,"age")||"—")}</span><span class="pill">लिंग: ${esc(get(v,"gender")||"—")}</span></div><div style="margin-top:8px;padding:8px 10px;background:var(--bg2);border-radius:12px;font-size:12px;line-height:1.4;"><div style="color:var(--p);font-weight:700;">मतदान केन्द्र: ${esc(get(v,"polling_station_no")||get(v,"part_no")+196)}</div><div style="color:var(--t2);">${esc(get(v,"polling_station_address")||get(v,"polling_station"))}</div></div><div style="display:flex;gap:8px;margin-top:10px;"><button type="button" class="whatsapp-btn" data-id="${esc(v.id)}" style="flex:1;padding:7px 10px;border-radius:10px;border:1px solid #25D366;background:#25D366;color:white;cursor:pointer;font-size:12px;font-weight:600;">WhatsApp</button><button type="button" class="view-pdf-btn" data-page="${esc(get(v,"pdf_page"))}" style="flex:1;padding:7px 10px;border-radius:10px;border:1px solid var(--p);background:var(--p);color:white;cursor:pointer;font-size:12px;font-weight:600;">पेज खोलें (#${esc(get(v,"pdf_page"))})</button><button type="button" class="print-slip-btn" data-id="${esc(v.id)}" style="flex:1;padding:7px 10px;border-radius:10px;border:1px solid var(--accent);background:var(--accent);color:var(--t);cursor:pointer;font-size:12px;font-weight:700;">🖨 Slip</button><button type="button" class="view-detail-btn" style="flex:1;padding:7px 10px;border-radius:10px;border:1px solid var(--b);background:transparent;color:var(--t);cursor:pointer;font-size:12px;">विवरण देखें</button></div></article>`).join(""):'<div class="empty">कोई परिणाम नहीं मिला</div>'}
$("#q").oninput=()=>{clearTimeout(t);t=setTimeout(search,120)};$("#go").onclick=search;$("#results").onclick=e=>{let wa=e.target.closest(".whatsapp-btn");if(wa){e.stopPropagation();const v=V.find(x=>String(x.id)===String(wa.dataset.id));if(v){const msg=`🗳️ वार्ड 16 मतदाता विवरण\n\nनाम: ${get(v,"name_hindi","name")||"—"}\nपिता/पति: ${get(v,"relation_name_hindi","relation")||"—"}\nभाग: ${get(v,"part_no","part")||"—"}\nक्रमांक: ${get(v,"serial_no","serial")||"—"}\nEPIC: ${get(v,"epic")||"—"}\nमकान: ${get(v,"house_no","house")||"—"}\nमतदान केन्द्र: ${get(v,"polling_station_no")||"—"}\nपता: ${get(v,"polling_station_address")||get(v,"polling_station")||"—"}\nPDF पेज: ${get(v,"pdf_page")||"—"}`;window.open(`https://wa.me/?text=${encodeURIComponent(msg)}`,"_blank");}return;}let printBtn=e.target.closest(".print-slip-btn");if(printBtn){e.stopPropagation();const v=V.find(x=>String(x.id)===String(printBtn.dataset.id));if(v)printSlip(v);return;}let pdfBtn=e.target.closest(".view-pdf-btn");if(pdfBtn){e.stopPropagation();let page=pdfBtn.dataset.page||1;window.open(`pdf/ward16.pdf#page=${page}`,"_blank");return;}let c=e.target.closest(".card");if(!c)return;current=V.find(v=>String(v.id)===String(c.dataset.id));if(!current)return;$("#dn").textContent=get(current,"name_hindi","name")||"नाम उपलब्ध नहीं";$("#de").textContent=get(current,"name_english","english_name");let f=[["पिता/पति का नाम",get(current,"relation_name_hindi","relation")],["पिता/पति (English)",get(current,"relation_name_english")],["संबंध प्रकार",get(current,"relation_type")],["लिंग",get(current,"gender")],["उम्र",get(current,"age")],["मकान नंबर",get(current,"house_no","house")],["EPIC",get(current,"epic")],["क्रमांक (Serial)",get(current,"serial_no","serial")],["भाग संख्या (Part)",get(current,"part_no","part")],["मतदान केन्द्र संख्या",get(current,"polling_station_no")],["मतदान केन्द्र पता",get(current,"polling_station_address")],["मतदान केन्द्र",get(current,"polling_station")],["कॉलोनी / क्षेत्र",get(current,"colony")],["PDF Page",get(current,"pdf_page")]];$("#details").innerHTML=f.map(x=>`<div class="detail"><small>${x[0]}</small><b>${esc(x[1]||"उपलब्ध नहीं")}</b></div>`).join("");$("#dlg").showModal()};$("#close").onclick=()=>$("#dlg").close();$("#pdf").onclick=()=>{if(current)window.open(`pdf/ward16.pdf#page=${parseInt(current.pdf_page,10)||1}`,"_blank")};

function printSlip(v){
 const area=$("#printSlipArea");
 if(!area||!v)return;
 const val=(...keys)=>esc(get(v,...keys)||"—");
 const part=val("part_no","part"), serial=val("serial_no","serial");
 const psNo=val("polling_station_no") || (get(v,"part_no")?String(get(v,"part_no"))+196:"—");
 area.innerHTML=`<div class="print-slip">
   ...
   <div class="thermal-section"><div class="thermal-label">मतदान केन्द्र</div><div class="thermal-psno">केन्द्र संख्या: ${psNo}</div><div class="thermal-address">${val("polling_station_address","polling_station")}</div></div>
 ...
 setTimeout(()=>{window.print();setTimeout(()=>{document.body.classList.remove("printing-slip");area.setAttribute("aria-hidden","true");},500);},50);
}
